const dataArr = [{"id":1,"firstName":"Osmond","surName":"Kieran","sex":"Male","age":26,"isVegetarian":true,"langCount":8},
...
let sumAge = 0
let maxLang = newArr[0].langCount
let male = 0
let female = 0
newArr.forEach(data => {
  sumAge += data.age
  if(data.langCount > maxLang) {
    maxLang = data.langCount
  }
  if(data.sex == 'Male') {
    male++
  } else {
...
const midAge = sumAge / newArr.length
document.getElementById('midAge').innerHTML = `Middle age is: ${midAge}`
document.getElementById('maxLang').innerHTML = `Middle amount of languages is: ${maxLang}`
document.getElementById('sex').innerHTML = `There are ${male} men and ${female} women in the list`
